...
const allProducts = [
  { id: 1, name: "Belgian Heirloom Flax", category: "LINEN", badge: "BEST SELLER", gsm: "180 GSM", desc: "Raw, unbleached, and highly breathable.", price: "$34.00", color: "#E0D8C3", roughness: 0.9, sheen: 0 },
  { id: 2, name: "Washed European Wexford", category: "LINEN", badge: "ESSENTIAL", gsm: "150 GSM", desc: "Pre-laundered for lived-in feel.", price: "$38.50", color: "#D1C7BD", roughness: 0.9, sheen: 0 },
  { id: 3, name: "Heavyweight Rustic Canvas", category: "LINEN", badge: "ARCHITECTURAL", gsm: "340 GSM", desc: "Substantial structural weave.", price: "$45.00", color: "#C6B99F", roughness: 0.8, sheen: 0 },
  { id: 4, name: "Delicate Sheer Gauze", category: "LINEN", badge: "NEW ARRIVAL", gsm: "90 GSM", desc: "An ethereal, translucent weave.", price: "$29.00", color: "#F0F0E8", roughness: 0.6, sheen: 0.1 },
  { id: 5, name: "Coastal Stripe Weave", category: "LINEN", badge: "LIMITED", gsm: "210 GSM", desc: "Crisp texture for summer tailoring.", price: "$32.00", color: "#DCD0B0", roughness: 0.7, sheen: 0 },
...
export default function Shop() {
  ...
  const categories = ["ALL", "LINEN", "SILK", "WOOL", "COTTON"]
  ...
  return (
    <main className="min-h-screen bg-[#F7F6F3] px-6 md:px-12 lg:px-24 py-12 md:py-16">
      {/* Header - Stacks on mobile, row on desktop */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
        <div>
          <h1 className="text-3xl md:text-4xl font-serif text-stone-900 mb-2">The Raw Catalog</h1>
          <p className="text-[10px] md:text-xs text-stone-500 tracking-wide uppercase">Pricing reflects cost per metric meter ($/m)</p>
        </div>
        
        {/* Filters - Wraps on small screens */}
        <div className="flex flex-wrap gap-2">
          {categories.map(cat => (
            <button 
              key={cat}
              onClick={() => setSelectedCat(cat)}
              className={`px-4 py-2 md:px-6 md:py-2 rounded-full text-[9px] md:text-[10px] font-bold tracking-widest transition-all ${
                selectedCat === cat 
      ...
      {/* Grid: 1 Col (mobile) -> 2 Col (tablet) -> 3 Col (large) -> 4 Col (desktop) */}
        ...
        <AnimatePresence mode='popLayout'>
          {filteredProducts.map(product => (
            <motion.div 
              ...
              className="bg-white p-5 border border-stone-200 flex flex-col hover:shadow-md transition-shadow"
            >
